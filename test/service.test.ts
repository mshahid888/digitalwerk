...
describe("chat agent service layer", () => {
  it("starts a session and persists it", async () => {
    ...
    const stored = await getChatAgentStore().conversations.get(sessionId);
  ...
  it("handles a message and records an analytics event", async () => {
    ...
    const events = await getChatAgentStore().events.list(sessionId);
  ...
  it("requestHandoff forces an escalation from current state", async () => {
    ...
    const result = await requestHandoff(sessionId, { note: "prefers a call" });
    expect("handoffId" in result).toBe(true);
    const handoffs = await getChatAgentStore().handoffs.list();
    expect(handoffs.length).toBeGreaterThanOrEqual(1);
    expect(handoffs.at(-1)?.payload.leadFacts.notes).toContain("prefers a call");
  });
